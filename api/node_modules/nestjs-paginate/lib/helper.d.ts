...
export declare type Column<T, D extends number = 2> = [D] extends [never] ? never : T extends Record<string, any> ? {
...
export declare type RelationColumn<T> = Extract<Column<T>, {
    [K in Column<T>]: K extends `${infer R}.${string}` ? R : never;
}[Column<T>]>;
export declare type Order<T> = [Column<T>, 'ASC' | 'DESC'];
export declare type SortBy<T> = Order<T>[];
export {};
