...
let UserService = class UserService {
    ...
    create(user) {
        return this.authService.hashPassword(user.password).pipe((0, rxjs_1.switchMap)((passwordHash) => {
            ...
            newUser.role = user_entity_1.UserRole.USER;
    ...
    paginate(options) {
        return (0, rxjs_1.from)((0, nestjs_typeorm_paginate_1.paginate)(this.userRepository, options)).pipe((0, rxjs_1.map)((userPageable) => {
    ...
    validateUser(email, password) {
        return this.findByEmail(email).pipe((0, rxjs_1.switchMap)((user) => this.authService.comparePassword(password, user.password).pipe((0, rxjs_1.map)((match) => {
            if (match) {
                const { password } = user, result = __rest(user, ["password"]);
                return result;
            }
